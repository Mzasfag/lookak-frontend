...
export const providerGuard: CanActivateFn = (_route, _state) => {
  ...
  try {
    ...
    // Approval gate: only fully approved providers may manage the salon.
    if (userData.providerStatus && userData.providerStatus !== 'approved') {
      return router.createUrlTree(['/forbidden']);
    }
