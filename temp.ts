    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ provider, services }) => {
          ...
          // Extract portfolio images from provider data
          const portfolio = providerData.portfolio || [];
          const legacyImages = (providerData.portfolioImages || []).map((url: string, idx: number) => ({
            url,
            publicId: `legacy-${idx}`,
            caption: '',
          }));
          this.portfolioImages.set([...portfolio, ...legacyImages]);
        },
        error: () => {
          this.hasError.set(true);
